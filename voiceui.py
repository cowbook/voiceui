...
from PySide6.QtGui import (
    ...
    QShortcut, QKeySequence, QKeyEvent, QTextCursor,
)
...
def _looks_like_placeholder_key(key: str) -> bool:
    ...
    if v.startswith("YOUR_"):
        return True
    return "DASHSCOPE_API_KEY" in v


def _load_local_env_file(env_path: Path = ENV_FILE) -> None:
...
def _upsert_env_var(env_path: Path, key: str, value: str) -> None:
    """Create or update KEY="VALUE" in .env while preserving other lines."""
...
def _ensure_api_key_with_prompt() -> bool:
    """Ensure DASHSCOPE_API_KEY exists (from env/.env or prompt user to input/save)."""
    existing = (os.environ.get("DASHSCOPE_API_KEY") or "").strip()
    if existing and not _looks_like_placeholder_key(existing):
        return True

    alias_key = (os.environ.get("ALIYUN_API_KEY") or "").strip()
    if alias_key and not _looks_like_placeholder_key(alias_key):
        os.environ["DASHSCOPE_API_KEY"] = alias_key
        return True

    QMessageBox.information(
        None,
        "需要配置 API Key",
        "首次启动需要配置阿里云百炼 API Key。\n"
        "请输入 DASHSCOPE_API_KEY，程序会保存到本地 .env，后续自动读取。",
    )

    while True:
        key, ok = QInputDialog.getText(
            None,
            "输入 DASHSCOPE_API_KEY",
            "请输入 API Key（示例: sk-...）:",
            QLineEdit.Password,
        )
        if not ok:
            QMessageBox.critical(None, "缺少 API Key", "未输入 API Key，程序将退出。")
            return False

        key = key.strip()
        if _looks_like_placeholder_key(key):
            QMessageBox.warning(None, "Key 无效", "请输入有效的 DASHSCOPE_API_KEY。")
            continue

        os.environ["DASHSCOPE_API_KEY"] = key
        try:
            _upsert_env_var(ENV_FILE, "DASHSCOPE_API_KEY", key)
        except Exception as e:
            QMessageBox.warning(
                None,
                "保存 .env 失败",
                f"本次运行会继续使用该 Key，但写入 .env 失败：\n{e}",
            )
        return True
...
class AgentBackend(QObject):
    ...
    def __init__(self, session_key: str = SESSION_KEY, model: str | None = None):
        super().__init__()
    ...
    def _extract_reply(stdout: str) -> str:
        ...
        try:
            ...
            if isinstance(data, dict):
                picked = _pick_text(data)
                if picked:
                    return picked
        except Exception:
            pass

        # jsonl/混合日志：倒序找最后一个可解析对象
    ...
    def _run(self, text: str):
        try:
            ...
            # 某些 agent 禁止 --model override，自动降级重试一次（去掉 --model）
            if (
                r.returncode != 0
                and self.model
                and "model override" in (r.stderr or "").lower()
                and "not allowed" in (r.stderr or "").lower()
            ):
                retry_cmd = [
                    "openclaw", "agent",
                    "--session-key", self.session_key,
                ...
                r = subprocess.run(retry_cmd, capture_output=True, text=True, timeout=180)

            if r.returncode != 0:
                err = (r.stderr or "").strip() or (r.stdout or "").strip() or "openclaw 返回非零退出"
                self.error.emit(f"agent 调用失败: {err}")
                return

            reply = self._extract_reply(r.stdout)
            if not reply:
                reply = "(空回复)"
...
class VoiceUIMain(QMainWindow):
    ...
    def __init__(
        ...
        self._ptt_active = False
        self._closing = False
        self._tts_watchdog: QTimer | None = None
    ...
    def _stylesheet(self) -> str:
        ...
        QWidget { color: #d8e8ff; font-family: 'Menlo','PingFang SC','Hiragino Sans GB'; }
        ...
        QTextEdit {
            ...
            font-family: 'Menlo','PingFang SC','Hiragino Sans GB';
        }
    ...
    def _check_tts_done(self):
        if self._closing:
            if self._tts_watchdog is not None:
    ...
    def _append(self, who: str, text: str, color: str = "#a0d0ff"):
        ...
        # 转义 HTML
        who_safe = str(who).replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
        safe = str(text).replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;").replace("\n", "<br/>")
        html = (
            f'<div style="margin:6px 0;">'
            f'<span style="color:{color}; font-weight:bold;">{who_safe}:</span> '
...
def main():
    ...
    if not _ensure_api_key_with_prompt():
        sys.exit(1)
